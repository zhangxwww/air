...
from models.baseline import Baseline_v2
...
def train_cluster(model, writer, stage, device, args):
    ...
    for epoch in range(args.epochs):
        ...
        for batch_idx, ((x, x_bar), label, _) in enumerate(train_loader):
            ...
            if args.ce_add:
                if args.ce_add_part:
                    pseudo_label = cluster_output.detach().max(1)[1]
                    loss_ce_add = w * criterion1(classify_output[:, target_list_lower:], pseudo_label) \
                                    / args.rampup_coefficient * args.increment_coefficient
                else:
                    pseudo_label = cluster_output.detach().max(1)[1] + target_list_lower
                    loss_ce_add = w * criterion1(classify_output, pseudo_label) \
                                    / args.rampup_coefficient * args.increment_coefficient
            ...
            if args.ewc_ce and stage > 0:
                ewc_loss = model.ewc_loss(device, stage) * args.ewc_lambda
                loss = loss + ewc_loss

                ewc_record.update(ewc_loss.item(), x.size(0))
        ...
        if args.ce_add:
            writer.add_scalar('Train Stage {}/Cluster/CE_Add Loss'.format(stage + 1), ce_add_loss_record.avg, epoch)
        if args.bce:
        ...
        with torch.no_grad():
            for batch_idx, (x, label, _) in enumerate(test_loader):
                x, label = x.to(device), label.to(device)
                classify_output, cluster_output, _ = model.ce_stage(x)
                targets = np.append(targets, label.cpu().numpy())
                preds = np.append(preds, classify_output.detach().max(1)[1].cpu().numpy())
        ...
        if args.pacc:
            ...
            acc_fun = partial(pred_2_gt_proj_acc, proj)
        else:
            acc_fun = cluster_acc

        acc = acc_fun(targets.astype(int), preds.astype(int))

        print('Test Cluster Stage {} Avg Acc: {:.4f}'.format(stage + 1, acc))
        writer.add_scalar('Test Stage {}/Cluster/Acc'.format(stage + 1), acc, epoch)

        # acc for labeled classes
        selected_mask = targets < args.num_labeled_classes
        acc_labeled = acc_fun(targets[selected_mask].astype(int), preds[selected_mask].astype(int))

        print('Test Cluster Stage {} Avg Acc for 0-{}: {:.4f}'.format(stage + 1, args.num_labeled_classes, acc_labeled))
        writer.add_scalar('Test Stage {}/Cluster/Acc for 0-{}'.format(stage + 1, args.num_labeled_classes), acc_labeled, epoch)
        ...
        for s in range(stage):
            ...
            acc = acc_fun(targets[selected_mask].astype(int), preds[selected_mask].astype(int))

            print('Test Cluster Stage {} Avg Acc for {}-{}: {:.4f}'.format(stage + 1, lower, upper, acc))
            writer.add_scalar('Test Stage {}/Cluster/Acc for {}-{}'.format(stage + 1, lower, upper), acc, epoch)

        # acc for cluster head:
        selected_mask = (targets >= target_list_lower) * (targets < target_list_upper)
        acc_cluster = acc_fun(targets[selected_mask].astype(int), cluster_preds[selected_mask].astype(int))
    ...
    save_dir = os.path.join(args.model_dir, 'stage_{}.pth'.format(stage + 1))
    torch.save(model.state_dict(), save_dir)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--unfix_last_layer', action='store_true', default=False)
    parser.add_argument('--unfix_all', action='store_true', default=False)
    parser.add_argument('--ft_ssl', action='store_true', default=False)
    parser.add_argument('--ft_ssl_level', type=int, default=4)
    parser.add_argument('--ft_ssl_stage', type=int, default=1)


    parser.add_argument('--pacc', action='store_true', default=False)
    ...
    for stage in range(4):
        ...
        # if not args.skip or args.start_stage <= stage:
        ...
        # if args.skip and args.start_stage == stage + 1:
