...
def train_ssl(model, writer, stage, device, args):
    lr = args.ssl_lr
    if stage > 0:
        lr *= args.inc_ssl_lr_decay
        lr *= args.inc_ssl_lr_decay_per_stage ** (stage - 1)
    ...
    if stage > 0:
        ...
        #     ]))
        if args.inc_ssl_data == 'exemplars':
            train_dataset = model.pseudo_labeling_and_combine_with_exemplars(None, target_list)
        ...
        elif args.inc_ssl_data == 'new_ins':
        ...
        elif args.inc_ssl_data == 'both':
            train_dataset = CIFAR100Data(root=args.dataset_root, split='train', aug='twice', target_list=target_list)
            train_dataset = model.pseudo_labeling_and_combine_with_exemplars(train_dataset, target_list)
            #train_dataset = TransformDataset(train_dataset, transform)
            train_loader = DataLoader(train_dataset, batch_size=args.batch_size, shuffle=True, num_workers=args.workers, pin_memory=True, drop_last=True)
    else:
        train_loader = CIFAR100Loader(root=args.dataset_root, batch_size=args.batch_size, split='train', aug='twice', shuffle=True, target_list=target_list)
    ...
    for epoch in range(epochs):
        ...
        if stage == 0 or args.inc_ssl_data == 'new_ins':
...
def train_ce(model, writer, stage, device, args):
    ...
    if args.print_cls_statistics:
        ...
        ytrue = targets.astype(int)
        ypred = proj[preds].astype(int)
        false = ytrue != ypred
        ytrue_f = ytrue[false]
        ypred_f = ypred[false]
        old_classes = target_list_lower
        old = ytrue_f < old_classes
        print('{} {}\n{} {}\n'.format(
            (ypred_f[old] < old_classes).sum(),
            (ypred_f[~old] < old_classes).sum(),
            (ypred_f[old] >= old_classes).sum(),
            (ypred_f[~old] >= old_classes).sum()))
        np.save('ytrue_{}.npy'.format(stage), ytrue)
        np.save('ypred_{}.npy'.format(stage), ypred)
