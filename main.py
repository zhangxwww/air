...
for epoch in range(args.epochs):
    ...
    for batch_idx, (data, target, _) in enumerate(train_loader):
        ...
        loss_nce_instance = (NCE(feature, args.nce_temperature)
                             if args.instance_nce else
                             torch.tensor(0)) * args.instance_nce_lambda

        loss_nce_structure = (NCE(z, args.nce_temperature)
                              if args.structure_nce else
                              torch.tensor(0)) * args.structure_nce_lambda

        loss_reconstruct = (F.binary_cross_entropy(adj_reconstructed, adj)
                            if args.reconstruct else
                            torch.tensor(0)) * args.reconstruct_lambda

        loss_modularity = (modularity_loss(output, adj) if args.modularity else
                           torch.tensor(0)) * args.modularity_lambda

        loss_bce = (F.binary_cross_entropy(adj_reconstructed.reshape(-1),
                                           WTA(output, args.wta_k))
                    if args.bce else torch.tensor(0)) * args.bce_lambda
    ...
    with torch.no_grad():
        ...
        writer.add_scalar('test-acc', total_correct / len(test_loader_labeled),
                          epoch)
