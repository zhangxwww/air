...
def train_cluster(model, writer, stage, device, args):
    ...
    for epoch in range(epochs):
        ...
        targets = targets - (args.test_stage - 1) * args.num_unlabeled_classes_per_stage
    ...
    for epoch in range(epochs):
        ...
        if not args.no_ce:
            ...
            for batch_idx, (x, pseudo) in enumerate(train_loader):
                ...
                label_in_new_classes = (pseudo >= target_list_lower) * (pseudo < target_list_upper)
                pseudo[label_in_new_classes] = pseudo[label_in_new_classes] - (stage - 1) * args.num_unlabeled_classes_per_stage
...
if __name__ == "__main__":
    ...
    parser.add_argument('--skip_model_dir', type=str, default='b3_firstssll_skipssl_s100lr1e2_cl100lr5e2_celr5e2_lwf_ft1/stage_1.pth')
    ...
    parser.add_argument('--test_stage', type=int)
    ...
    model.cuda()
    update_exemplars(model, 0, args)

    model.increment_classes(args.num_unlabeled_classes_per_stage, device)

    train_cluster(model, writer, args.test_stage, device, args)
    update_exemplars(model, args.test_stage, args)
    train_ce(model, writer, args.test_stage, device, args)
