...
from .cifarloader import PartialCIFAR100, PARTITION_CONFIG, CIFAR100
...
class GenericDataset(data.Dataset):
    def __init__(self, dataset_name, split, random_sized_crop=False,
        ...
        if self.dataset_name=='cifar10':
            ...
            transform.append(lambda x: np.asarray(x))
            self.transform = transforms.Compose(transform)
            self.data = CIFAR100(root=dataset_root, split=self.split, transform=self.transform, target_list=list(range(70)))
            # self.data = datasets.__dict__[self.dataset_name.upper()](
            #     dataset_root, train=self.split=='train',
            #     download=False, transform=self.transform)
            # self.data = PartialCIFAR100(root=dataset_root, split=self.split, transform=self.transform,
            #                             target_list=PARTITION_CONFIG['stage 1'][0][0],
            #                             partition=PARTITION_CONFIG['stage 1'][0][1])
            ...
            transform.append(lambda x: np.asarray(x))
            self.transform = transforms.Compose(transform)
            self.data = PartialCIFAR100(root=dataset_root, split=self.split, transform=self.transform, target_list=PARTITION_CONFIG['stage 1'][0])
            # self.data = datasets.__dict__[self.dataset_name.upper()](
